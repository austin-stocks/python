...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    datefmt='%m-%d %H:%M',
                    filename=dir_path + log_dir + "\\" + 'SC_Watchlist_Analysis_debug.txt',
                    filemode='w')
...
watchlist_xls = pd.ExcelFile(dir_path + user_dir + "\\" + watchlist_file)
watchlist_sheets = ['Z-Max', 'PocketPivot','WallofBlue','Molly','Barrons','IBDPaper','IBDIndGroup','ValueLine','Sundeep','Others']

df_name_list = []
for sheet in watchlist_sheets:
  logging.debug("Reading Sheet : " + str(sheet))
  df_name = sheet + "_watchlist_df"
  df_name = pd.read_excel(watchlist_xls, sheet)
  df_name_list.append(df_name)
  logging.debug("Sheet Name " + str(sheet) + "\n" + df_name.to_string())


sys.exit()

ibd_inds_gp_rank_df = pd.read_excel(dir_path + user_dir + "\\" + watchlist_file, sheet_name="Watchlist_01")
watchlist_ticker_list = ibd_inds_gp_rank_df['Tickers'].tolist()
logging.debug("The tickers in Watchlist are :  " + str(watchlist_ticker_list))


# TODO : get the watchlist ticker IBD numbers from latest and greatest IBD Data table
# Add all the stockcharts etc columns for it
# Now that I have Yahoo Finance modules, should I get Yahoo Finance data or
# Should I pull data from AAII xlxs download?
# Also look at AAII chart for it
# Should I also get Yahoo Historical data and look for Z-Max, Pocket Pivot etc
#   or do I get that from Stockcharts (if Stockchart would allow me to save
#   different formats of charts?
# Should I add some link for finviz too?
