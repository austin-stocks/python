...
for ticker_raw in ticker_list:
  ...
  earnings_df = pd.read_csv(earnings_file_fullpath)
  if 'Q_EPS_Projections_Date_0' in earnings_df.columns:
    earnings_file_update_list = earnings_df['Q_EPS_Projections_Date_0'].tolist()
    date_updated_eps_projections = earnings_file_update_list[0]
    print ("Column exists in ", earnings_file, "and updated on ",date_updated_eps_projections)
  else:
...
#
  ...
  date_updated_eps_projections_dt = dt.datetime.strptime(str(date_updated_eps_projections), '%m/%d/%Y').date()
...
#
  ...
  # ---------------------------------------------------------------------------
  if ((ticker_is_wheat == "Wheat") and pd.isnull(last_earnings_date)):
  ...
  if (not pd.isnull(last_earnings_date)):
    ...
    if (date_year < 2019):
      ...
      sys.exit()
    if (today > (last_earnings_date_dt  + dt.timedelta(days=100))):
